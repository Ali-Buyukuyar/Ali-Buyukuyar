...
def main() -> None:
    ...
    for date, level in ordered:
        ...
        class_name = "cell active" if level else "cell"
        cells.append(
            f'<rect class="{class_name}" style="--phase:{phase}s" x="{x}" y="{y}" '
...
.cell {{ opacity:.68 }}
.active {{ animation:matrix 6.4s linear infinite; animation-delay:calc(var(--phase) * -1); transform-origin:center; }}
@keyframes matrix {{
  0%, 76%, 100% {{ opacity:.72; filter:none; transform:scale(1); }}
  82% {{ opacity:1; filter:drop-shadow(0 0 4px #39d353); transform:scale(1.35); }}
  88% {{ opacity:.9; filter:drop-shadow(0 0 7px #39d353); transform:scale(1.15); }}
}}
</style>
